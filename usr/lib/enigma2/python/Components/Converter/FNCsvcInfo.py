...
class FNCsvcInfo(Converter, object):
    HAS_TELETEXT = 0
    IS_MULTICHANNEL = 1
    IS_CRYPTED = 2
    IS_WIDESCREEN = 3
    ...
    def __init__(self, type):
        ...
        self.type, self.interesting_events = {'HasTelext': (self.HAS_TELETEXT, (iPlayableService.evUpdatedInfo,)),
         ...
         'VideoXY': (self.XRES2, (iPlayableService.evVideoSizeChanged,))}[type]
    ...
    def getText(self):
        ...
        if self.type == self.XRES2:
            return self.getServiceInfoString(info, iServiceInformation.sVideoWidth) + ' x ' + self.getServiceInfoString(info, iServiceInformation.sVideoHeight) + ' @ ' + self.getServiceInfoString(info, iServiceInformation.sFrameRate, lambda x: '%d fps' % ((x + 500) / 1000))
